...
'''' ############ Train Epoch for Resnext101 + META ############ '''
def train_epoch_META(model_encoder, model_meta, model_classifier, loader, optimizer, criterion):
    ...
    # adding progress bar for easier monitoring during training
    bar = tqdm(loader)
    for (data, meta, target,_,_) in bar:
        # zeroing gradients
        optimizer.zero_grad()
        ...
        # predicting with model and getting feature maps and logits
        feat_out = model_encoder(data)  # creating feature representation using the encoder
        meta_out = model_meta(meta)
        
        logits = model_classifier(feat_out,meta_out)  # using the main classifier to get output logits        
        target = target.unsqueeze(1).type_as(logits)  # unsqueezing to [batch_size,1] and same dtype as logits
...
''' ########### Train Epoch for Resnext101 + META + TABE'''
def train_epoch_TABE_META(model_encoder, model_meta, model_classifier, model_aux, loader, optimizer, optimizer_aux,
                     optimizer_confusion, criterion, criterion_aux):
    ...
    # adding progress bar for easier monitoring during training
    bar = tqdm(loader)
    for (data, meta,target, target_aux, target_aux2) in bar:
        if args.rulers:  # switching targets round if wanting to use rulers as target
            target_aux = target_aux2
        # zeroing gradients
        optimizer.zero_grad()
        ...
        # sending data and targets to cpu
        data, meta, target, target_aux = data.to(device), meta.to(device), target.to(device), target_aux.to(device)
        # predicting with model and getting feature maps and logits
        feat_out = model_encoder(data)  # creating feaure representation using the encoder
        meta_out = model_meta(meta)
        logits = model_classifier(feat_out,meta_out)  # using the main classifier to get output logits
