...
from domain_specific_augmentation import *
...
class SIIMISICDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.args.META:
            #meta_data = find_global_features(image.astype(np.int32))
            meta_data = find_global_features(image) 
            meta = torch.tensor(meta_data).float()
        ...
        # Performing Domain Specific Augmentation 
        if self.mode == 'train' and self.args.domainAug:
            GT = cv2.imread(row.mm_masks)
            GT = cv2.bitwise_not(GT)
            
            #aug_prob = random.randint(1,3)
            aug_prob = 1
            ...
            if aug_prob == 2: #ruler augmentation
                ruler_array = np.load('ruler_array.npy')
                image = ruler_aug(image,GT,ruler_array)
                
            if aug_prob == 3: #marker augmentation
                mms = np.load('mm_array.npy')  
                image = mm_aug(image,GT,mms)
        ...
        if self.mode == 'test':
            if self.args.META:
                return data, meta, torch.tensor(self.csv.iloc[index].target).long()
        ...
        # Returning different data based on what test is being run
            ...
            if self.args.instrument:
                ...
                return data, meta, torch.tensor(self.csv.iloc[index].target).long(),  torch.tensor(
...
#blanks rulers and marked not required as we do not have access to private dataset